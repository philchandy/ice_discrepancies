...
def calculate_distances_traveled_original(df):
    """
    ...
    facility_code_map = pd.read_csv(r"lookups/facility_code_map_with_coordinates.csv", encoding="latin1")
    ...
    def dissect_code_distance(code_list):
        ...
        for i in range(len(code_list) - 1):
            start = code_to_lat_long_lookup.get(code_list[i])
            end = code_to_lat_long_lookup.get(code_list[i + 1])
...
def map_most_serious_conviction(df):
    ...
    # Insert columns immediately to the right of detention_release_reason
    source_col_index = df.columns.get_loc("most_serious_conviction_charge")

    columns = list(df.columns)
    columns.insert(source_col_index + 1, columns.pop(columns.index("most_serious_conviction_charge_general")))
...
def clean_detention_stays(raw_df):
    ...
    detention_stays_df["facility_codes_all_collapsed"] = detention_stays_df["facility_codes"].apply(list_collapse_all_codes)
    detention_stays_df["facility_codes_with_middle_duplicates_collapsed"] = detention_stays_df["facility_codes"].apply(list_collapse_middle_codes)
    ...
    detention_stays_df["number_transfers_during_stay"] = detention_stays_df["facility_codes_all_collapsed"].apply(lambda x: len(x) - 1)

    # Add in the stint tuples
    results = [create_stint_tuples(row) for _, row in detention_stays_df.iterrows()]
...
# Get the data in console:
clean_detention_stays_df = build_and_pickle_detention_stays()
